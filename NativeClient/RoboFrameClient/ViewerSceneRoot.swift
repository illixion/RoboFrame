/*
 ...
 visionOS-only. Backs the "slideshow-viewer" `WindowGroup(for: UUID.self)`
 scene added so multiple slideshow/web-page windows can be open at once —
 mirrors Hypnos's `RemoteViewerSceneRoot`, which resolves a launched window's
 profile by id from the shared store rather than carrying the profile value
 itself through the scene. Resolving by id (instead of making
 `RoboFrameProfile` the scene's `Codable` payload) means an ornament-driven
 edit to the profile is picked up live in the open window, the same as it is
 in `ProfileManagerView`'s own `.fullScreenCover` path.
...
struct ViewerSceneRoot: View {
    ...
    var body: some View {
        ...
        if let profileID, let profile = store.profiles.first(where: { $0.id == profileID }) {
