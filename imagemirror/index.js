...
//
//   "dc,rank[,seed]"  the random deck's (display_count, rank) tuple plus the
//              caller's ordering seed — what `nextCursor` carries in the
//              default random order, so a client walking a multi-page result
//              feeds it straight back.
//   "N"        a bare number: the row offset for `order:id|score|score_asc`,
//              and in random order a seed — the page opens on the caller's
//              own permutation of the least-seen tier, the seeding trick a
//              one-shot client uses to land on an arbitrary window (and the
//              same one every orchestrator channel uses).
function parseSearchCursor(raw) {
  if (raw === undefined || raw === null || raw === '') return null;
  const str = String(raw);
  if (str.includes(',')) {
    const [dcStr, rankStr, seedStr] = str.split(',');
    ...
    if (seedStr !== undefined && Number.isFinite(seed)) cursor.seed = seed;
    return cursor;
  }
  const n = Number(str);
  if (!Number.isFinite(n)) return null;
  return { seed: n, offset: Math.max(0, Math.floor(n)) };
}
...
//   cursor= paging position. In the default random order this is a bare
//           number — a seed for the caller's own ordering of the deck: the
//           page opens on that permutation of the least-seen tier, the same
//           seeding trick each orchestrator channel uses so two displays
//           coming up together don't both get the deck's head. A
...
//           `dc,rank,seed` (random order) or a bare offset (deterministic
