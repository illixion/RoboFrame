...
`GET /browse?lowmem=`
HTML page for searching the library: a query box over a contact-sheet grid of every matching post at its true aspect ratio. Pages in as you scroll (`/search` + its `nextCursor`) — a page that lands without reaching past the fold pulls the next one itself, and a **Load more** button at the tail is both the manual override and the retry after a failed page. Posts are deduplicated on arrival, since the deck's `(display_count, random_rank)` paging key moves whenever the displays advance a frame. Counts matches with `/count`, and lazy-loads each thumbnail from `/get` a few at a time so a wide query doesn't fire hundreds of conversions at once. Clicking a frame opens it full-size with its DuckDB row alongside — dimensions, ratio, rating, score, favourites, length — plus its tags as buttons that search them, and Save / Block / Download. The viewer hands the **source file** to the browser first (`rawanimated=1`, so no server-side conversion runs at all) and only falls back to the converted variant when the browser can't decode it — so a JXL library displays its own bytes in Safari while Chrome silently gets JPEG; the rail says which of the two you're looking at. **Download** always saves the untouched original under `<id>.<ext>`, whatever the browser can display. Arrow keys walk the sheet, Esc or a click beside the frame closes it, `/` jumps to the query box, and the query lives in the page URL so a search is shareable. Video posts preview from their first frame; animated posts play their converted mp4 in the grid (the fetched `Content-Type` picks the renderer, not the post's `ext`). The **Tag lists** dropdown offers each server-side tag list (`data.json`'s `tagLists`, read from `/rpc/tags.json`) as a preset — one line per list showing its whole space-separated query — and drops the chosen one into the box as editable text; it reads as selected only while the query still matches, and the control is left out entirely when no lists are configured. *Random window* restarts the results from an arbitrary point in the deck. `lowmem=1` propagates into the in-page `/get` URLs for Pi-class browsers. Links across to `/history`, carrying the same token.
...
`cursor=` is the paging position. In the default random order it's a bare float in `[0, 1)` naming a starting point in the frozen `random_ranks` deck — the same seeding trick each orchestrator channel uses so two displays coming up together don't both get the deck's head. **A fresh `Math.random()` per call is how a one-shot client gets an arbitrary window of `N` distinct posts in a single request** — e.g. `/search?q=&limit=9&cursor=0.4137` for an iOS Shortcut rendering a grid of nine (each result's `_id` then goes to `/get?id=`). For `order:id|score|score_asc` the same param is a row offset instead. Feed the returned `nextCursor` back for the following page — as `dc,rank` in random order, or a bare number for `{ offset }`; a short page returns `null`.
