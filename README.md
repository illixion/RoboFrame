...
| `ACCESS_TOKEN` | `accessToken` *(top-level)* | *(required)* | Read-mostly token shared by every consumer (kiosks, node-display, Hypnos). Required to connect to `/rpc/ws` (kiosk tier — broadcasts + kiosk-scoped actions, not `rpcsend`) and to call the image API routes (`/get`, `/save`, `/history`, `/browse`, `/addtohistory`, `/rpc/tags.json`) via `?token=` or the `X-RoboFrame-Token` header. Top-level so the same value can be copied to client configs. Must differ from `rpcToken`. |
