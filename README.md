...
`GET /browse?lowmem=`
HTML page for searching the library: a query box over a contact-sheet grid of every matching post at its true aspect ratio. Pages in as you scroll (`/search` + its `nextCursor`), counts matches with `/count`, and lazy-loads each thumbnail from `/get` a few at a time so a wide query doesn't fire hundreds of conversions at once. Clicking a frame opens it full-size with its DuckDB row alongside — dimensions, ratio, rating, score, favourites, length — plus its tags as buttons that search them, and Save / Block. Arrow keys walk the sheet, `/` jumps to the query box, and the query lives in the page URL so a search is shareable. Video posts preview from their first frame; animated posts play their converted mp4 in the grid (the fetched `Content-Type` picks the renderer, not the post's `ext`). *Random window* restarts the results from an arbitrary point in the deck. `lowmem=1` propagates into the in-page `/get` URLs for Pi-class browsers. Links across to `/history`, carrying the same token.
